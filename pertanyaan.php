...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Load questions and dares from localStorage
            ...
            saveEditButton.addEventListener('click', function () {
                const newQuestion = editQuestionInput.value;
            ...
            document.getElementById('addTruthForm').addEventListener('submit', function (event) {
                ...
                const question = document.getElementById('truthQuestion').value;
                addQuestion('truth', question);
                document.getElementById('addTruthForm').reset();
                const addTruthModal = bootstrap.Modal.getInstance(document.getElementById('addTruthModal'));
                addTruthModal.hide();
                saveToLocalStorage();
            });
            ...
            document.getElementById('addDareForm').addEventListener('submit', function (event) {
                event.preventDefault();
                const question = document.getElementById('dareText').value;
                addQuestion('dare', question);
                document.getElementById('addDareForm').reset();
                const addDareModal = bootstrap.Modal.getInstance(document.getElementById('addDareModal'));
                addDareModal.hide();
                saveToLocalStorage();
            });
            ...
            function saveToLocalStorage() {
                const truthCards = document.querySelectorAll('#truth-list .custom-card');
                const dareCards = document.querySelectorAll('#dare-list .custom-card');

                const truthQuestions = Array.from(truthCards).map(card => ({
                    type: card.dataset.type,
                    question: card.dataset.question
                }));

                const dareQuestions = Array.from(dareCards).map(card => ({
                    type: card.dataset.type,
                    question: card.dataset.question
                }));

                const allQuestions = truthQuestions.concat(dareQuestions);
            ...
            function loadFromLocalStorage() {
                ...
                questions.forEach(question => {
                    addQuestion(question.type, question.question);
                });
            }
        });     
    </script>
